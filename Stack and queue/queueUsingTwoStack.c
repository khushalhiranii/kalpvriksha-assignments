...
void dequeue(Stack *stack) {
    ...
    Stack *duplicateStack = (Stack *)malloc(sizeof(Stack));
    duplicateStack->head = NULL;

    while (size(stack) > 1) {
        push(duplicateStack, pop(stack));
    }

    printf("Dequeued: %d\n", pop(stack));

    while (!isEmpty(duplicateStack)) {
        push(stack, pop(duplicateStack));
    }
